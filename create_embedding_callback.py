...
class my_callback_each_epoch(CallbackAny2Vec):
    ...
    def on_epoch_begin(self, model):
        ...
        if self.epoch>=6:
            if self.loss_list[-1] < max(self.loss_list[len(self.loss_list)-6:-1]) :
                print('exiting')
    ...
    def on_epoch_end(self, model):
        ...
        self.loss_list.append(model.running_training_loss)
...
def create_embedding_scratch_fastText():
    epoch_logger = my_callback_each_epoch([])
    fastText_model = Word2Vec(common_texts, iter=100, size=10, min_count=0, seed=42, callbacks=[epoch_logger],compute_loss=True) #FastText(common_texts, size=4, window=3, min_count=1, iter=10,callbacks=[call])
    #saver = EpochSaver("my_w2v")
    #model = Word2Vec(common_texts, iter=5, size=10, min_count=0, seed=42, callbacks=[saver])
    #print(fastText_model.running_training_loss)
...
if __name__=='__main__':
    ...
    create_embedding_scratch_fastText()

#--Initial_Embedding test.txt --Corpus_file /media/gaurav/Elements/Thesis/data/mappedData/mappedData/1865_1980/2s.txt --Model_name 'test' --min_count 1 --workers 1
